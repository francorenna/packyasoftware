...
const outputDir = path.join(projectRoot, 'release', 'qa-order-docs-polish-final-preview')
...
const loadKit = async () => {
  ...
  const [header, footer, watermark] = await Promise.all([
    ...
    fs.readFile(path.join(base, 'watermark.svg'), 'utf8'),
  ])
...
const buildOrder = (itemCount) => {
  ...
  return {
    ...
    status: itemCount >= 20 ? 'En ProducciÃ³n' : itemCount >= 10 ? 'Listo para Entrega' : 'Pendiente',
